...
/*------------------------MÉTODO PARA DEFINIR ID------------------------------*/
    public void setId(String id) {
        this.id = id;
    }
    
/*-----------------CONSTRUTOR DA CLASSE PERGUNTA CRIADA-----------------------*/
...
/*---------------------MÉTODO PARA VERIFICAR RESPOSTA-------------------------*/
    public void verificarResposta(String alternativaSelecionada, 
        ...
        try (Connection conn = ConexaoBD.obterConexao();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    switch (rs.getInt("respostaCriada")) {
                        case 1:
                            alternativaCorretaBD = 
                                    rs.getString("alternativaA").trim();
                            break;
                        case 2:
                            alternativaCorretaBD = 
                                    rs.getString("alternativaB").trim();
                            break;
                        case 3:
                            alternativaCorretaBD = 
                                    rs.getString("alternativaC").trim();
                            break;
                        case 4:
                            alternativaCorretaBD = 
                    ...
                    System.out.println("Alternativa selecionada: '" 
                            + alternativaSelecionadaTexto + "'");
...
/*---------------------MÉTODO PARA CARREGAR NOVA QUESTÃO----------------------*/
    public static void carregarNovaQuestao(PanelPergunta panel, 
            String idProfessor) throws Exception {
...
/*----------------MÉTODO PARA LIMPAR E INSERIR NOVA QUESTÃO-------------------*/
    public static void limparEInserirQuest(PanelPergunta panel, 
            String idProfessor) throws Exception {
...
/*-------------MÉTODO PARA OBTER TOTAL DE PERGUNTAS DISPONÍVEIS---------------*/
    private static int getTotalPerguntasDisponiveis() throws Exception {
        try (Connection conn = ConexaoBD.obterConexao();
             PreparedStatement stmt = conn.prepareStatement
        ("SELECT COUNT(*) AS total FROM alternativascriadas");
        ...
        (null, "Erro ao contar perguntas: " + e.getMessage());
            throw e;
        }
    }
}